...
fea.grid.b(3,:) = 1;
[~,ix] = findbdr( fea, 'x<=sqrt(eps)', 0 );
fea.grid.b(3,ix) = 2;
[~,ix] = findbdr( fea, 'x>=2.5-sqrt(eps)', 0 );
fea.grid.b(3,ix) = 3;
[~,ix] = findbdr( fea, 'sqrt((x-0.5).^2+(z-0.21).^2)<(0.05+sqrt(eps))', 0 );
fea.grid.b(3,ix) = 4;
fea.grid.s(:) = 1;
...
% Boundary conditions.
fea.phys.ns.bdr.sel(2) = 2;
fea.phys.ns.bdr.sel(3) = 4;
fea.phys.ns.bdr.coef{2,end}{1,2} = ['16*',num2str(opt.uin),'*(y*z*(0.41-y)*(0.41-z))/0.41^4'];
