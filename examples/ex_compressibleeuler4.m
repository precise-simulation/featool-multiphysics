...
fea.phys.ee.bdr.sel(4) = 1;
fea.phys.ee.bdr.coef{1,end}{1,4} = rho0;
fea.phys.ee.bdr.coef{1,end}{2,4} = u0;
fea.phys.ee.bdr.coef{1,end}{3,4} = v0;
fea.phys.ee.bdr.coef{1,end}{4,4} = p0;
...
elseif( strcmp(opt.solver,'su2') )
  logfid = fid; if( ~got.fid ), fid = []; end
  fea.sol.u = su2( fea, 'upwind', 'jst', 'fid', fid, 'logfid', logfid );
  fid = logfid;
else
