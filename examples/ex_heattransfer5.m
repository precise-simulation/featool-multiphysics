...
%   through a surrounding medium kept at 17 C and a heat transfer coefficient
%   of 750 W/m^2K [1].
%
%   Reference:
%
...
k_steel      =    44;
k_tungsten   =   163;
fea.phys.ht.eqn.coef{1,end} = { rho_tungsten rho_steel rho_tungsten };   % Density
fea.phys.ht.eqn.coef{2,end} = { cp_tungsten  cp_steel  cp_tungsten  };   % Heat capcity.
fea.phys.ht.eqn.coef{3,end} = { k_tungsten   k_steel   k_tungsten   };   % Thermal conductivity.
fea.phys.ht.eqn.coef{7,end} = { 84 -10 84 };                             % Initial temperature.

% Boundary conditions.
n_bdr = max(fea.grid.b(3,:));
fea.phys.ht.bdr.sel(fea.phys.ht.bdr.sel>0) = 4;
for i_bdr=1:n_bdr
  fea.phys.ht.bdr.coef{4,end}{i_bdr}{2} = 'k_ht';
