...
% Model parameters.
rho       = opt.rho;     % Density.
miu       = [num2str(opt.mu0*opt.K),'*(sqrt(2)*sqrt(1/2*(4*ux^2+(uy+vx)^2+4*vy^2)+eps))^(',num2str(opt.n-1),')'];     % Effective viscosity.
...
fea.phys.ns.eqn.coef{end,end}{1} = [num2str(opt.dp),'*(1-x/',num2str(l),')'];   % Initial perssure.
fea.phys.ns.sfun = { sf_u sf_u sf_p };           % Set shape functions.
fea.phys.ns.bdr.sel([i_inflow,i_outflow]) = 4;
fea.phys.ns.bdr.coef{4,end}{3,i_inflow}   = l/opt.dp;   % Set inflow pressure.
