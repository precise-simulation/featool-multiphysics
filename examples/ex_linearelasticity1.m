...
%
%   [ FEA, OUT ] = EX_LINEARELASTICITY1( VARARGIN ) Example to calculate displacements and
%   stresses for a cube stretched in one direction.
%
...
out.peerr  = [];
for i=1:6
  out.serr(i) = norm( abs( evalexpr( fea.phys.el.eqn.vars{5+i, end}, p, fea ) - (i==opt.idir) ) );
  out.eerr(i) = norm( abs( evalexpr( fea.phys.el.eqn.vars{14+i,end}, p, fea ) - (i==opt.idir) + 0.3*((i<=3 & i~=opt.idir)) ) );
  if( opt.psecheck && i<=3 )
