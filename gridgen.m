...
%      grid = gridgen( geom, 'hmax', 0.1, 'quad', true );
%      plotgrid( grid )
%
...
%      hmaxb = hmax*ones(1,4);
%      hmaxb(3) = hmax/5;
%      grid  = gridgen( geom, 'hmax', hmax, 'hmaxb', hmaxb );
%      plotgrid( grid )
%
...
%
%      grid  = gridgen( geom, 'hmax', [0.0025 0.05 0.0025] );
%      plotgrid( grid )
%
...
%      hmaxb = zeros(1,21);
%      hmaxb([5 20]) = w/50;
%      grid = gridgen( geom, 'hmax', w./[5 5 20], 'hmaxb', hmaxb );
%      plotgrid( grid )
